...
### Временная сложность:
O(G · P · n²) 
Доминирующий член — O(G · P · n²) из‑за кроссовера (OX).

Внутри кроссовера поиск города в потомке занимает O(n) на город, а городов n, поэтому O(n²) на особь.

Оптимизация кроссовера: если использовать хеш‑таблицу для отслеживания уже добавленных городов, можно снизить сложность кроссовера до O(n) на особь, тогда общая сложность станет O(G · P · n).
​
### Контрольный вопрос

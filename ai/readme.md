# Нейронная сеть для определения качества пищи
...
🎯 ИТОГИ:
  ✓ Точность нейросети TensorFlow: 93.42%
  ✓ ROC-AUC нейросети: 0.9456
  ✓ Лучший алгоритм: TensorFlow NN
  ✓ Лучшая точность: 93.42%
  ✓ Использовано 5 алгоритмов для сравнения
```
...
### 1. Цикл машинного обучения
```
Данные → Предобработка → Модель → Обучение → Оценка → Результаты
   ↑                                                    ↓
   └────────────────────← Улучшение ←─────────────────┘
```
...
### 2. Трен vs Тест
```
Тренировочные данные (5,197):
- Используются для обучения модели
- Модель "видит" эти данные
- Accuracy обычно выше

Тестовые данные (1,300):
- Используются для оценки
- Модель первый раз видит эти данные
- Accuracy обычно немного ниже (переобучение?)
- Это как реальная производительность
```
...
- Ранняя остановка (остановить обучение вовремя)
- Больше данных
```

### 4. Почему несколько алгоритмов?
```
Разные алгоритмы имеют разные свойства:
- Логрегрессия: простая, быстрая, интерпретируемая
- Random Forest: хорошие результаты, параллелизируется
- XGBoost: часто лучшие результаты, медленнее
- Neural Network: очень гибкая, требует много данных

В реальности: часто используют ансамбль (голосование нескольких моделей)
```
...
### Из 53 блоков кода это делает:

1. **Установка** (1 блок) - загрузить библиотеки
2. **Импорты** (1 блок) - подключить функции
3. **Загрузка** (1 блок) - получить данные из интернета
4. **Анализ** (1 блок) - визуализировать распределение
5. **Предобработка** (1 блок) - очистить и подготовить
6. **Нейросеть** (2 блока) - определить архитектуру и обучить
7. **Оценка NN** (2 блока) - графики и метрики
8. **Сравнение** (5 алгоритмов) (3 блока) - обучить и оценить
9. **Результаты** (2 блока) - таблицы и ROC-кривые

**Итог**: полный цикл ML - от сырых данных до выводов в 53 блоках кода!
...
1. Копируем весь код
2. Открываем https://colab.research.google.com
3. Создаем новый notebook
4. Вставляем код
5. Нажимаем Shift+Enter для каждого блока (или Ctrl+F9 для всего)
6. Смотрим результаты и графики

**Время выполнения**: ~5-10 минут на GPU (T4 в Colab бесплатно)
